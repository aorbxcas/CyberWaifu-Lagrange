...
from waifu.Tools import get_photo_from_tieba
import logging
...
class Waifu():
    ...
    def __init__(self,
        ...
        self.use_photoFinder = use_photoFinder
        self.used_photoFinder = False

        self.tools = Tools(self, search_api, use_search)
    ...
    def ask(self, text: str) -> str:
        ...
        # 启用图片爬虫
        if self.use_photoFinder:
            judge_bot = ChatOpenAI(
                openai_api_key=self.openai_api_key,
                model='gpt-3.5-turbo',
                temperature=0,
            )
            judge_message = [SystemMessage(
                content="你擅长分析他人的需求，现在分析下面的句子判断对方是否有可能想要图片，注意，你只能回答yes或no，如果有就回答yes，如果没有就回答no，",
                additional_kwargs={}),
                message]
            judge_result = judge_bot(judge_message).content
            print("AI判断是否需要启用图片爬虫功能，结果为：" + judge_result)
            if judge_result == "yes":
                self.used_photoFinder = True
                # 发送消息
                self.chat_memory.messages.append(message)
                self.history.messages.append(message)
                messages.extend(self.chat_memory.messages)
                messages.append(SystemMessage(content="你成功找到一张适合的图片并发送给了哥们，期待哥们的看法",
                                              additional_kwargs={}))
        ...
        while self.brain.llm.get_num_tokens_from_messages(messages) > 4096:
            self.cut_memory()
        logging.debug(f'LLM query')
        reply = self.brain.answer_agent(messages)
        # self.brain.answer_agent(messages)
    ...
    def finish_ask(self, text: str) -> str:
        ...
        if self.use_emoticon:
            if self.used_photoFinder:
                self.used_photoFinder = False
                file = get_photo_from_tieba()
                file = './waifu/' + file
                logging.info(f'发送图片: {file}')
                return file
            else:
