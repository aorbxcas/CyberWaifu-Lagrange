...
class QuestionGenerate():
    ...
    def __init__(self, brain: Brain, probability=0.4):
        ...
        self.role = '''你是一个用于将问题总结并转述给他人的系统，请将输入的文字总结成一个问题，请注意代词的转换'''

    def think(self, text: str):
        message = [
            SystemMessage(content=self.role),
            HumanMessage(content=f'Summarize the following text into a question:\n"{text}"')
        ]
...
class AddEmoji():
    ...
    def __init__(self, brain: Brain, probability=0.2):
        self.brain = brain
        self.probability = probability
