...
from qqbot.socket_qqbot import socket_qqbot
import configparser
...
use_photoFinder = str2bool(config['Thoughts']['use_photoFinder'])
voice = config['TTS']['voice']
openai_api_key = config['LLM_OpenAI']['openai_key']
...
if model == 'OpenAI':
    ...
    brain = GPT(openai_api, name, stream=True, callback=callback, base_url=base_url)
else:
    callback = None
...
waifu = Waifu(brain=brain,
              ...
              use_photoFinder=use_photoFinder,
              openai_api_key=openai_api_key,
              base_url=base_url)

brain.set_tools(waifu.tools)
...
qqbot = socket_qqbot(waifu=waifu, send_text=send_text)
qqbot.def_socket_thread()
